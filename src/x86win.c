/*
 * Tiny Constructive Compiler (TCC) - x86 code generator (Windows NT)
 *
...
static void addsubop(char *__codestr)
{
    if(_op1 && _op2)    //op1 and op2 both are variables
                ...
                switch(_symtab[__var].bucket->type)
                {
                    ...
                    case STR:  if(_symtab[__var].bucket->source==USER && !_symtab[__var].bucket->ispointer)
                               ...
                               else
                               {
                                   ...
                                   _symtab[__var].bucket->val[strlen(_symtab[__var].bucket->val)-1]=0;
                                   fprintf(_outfile,"\n\t.ascii\t%s\\0\"",_symtab[__var].bucket->val);  //.string directive
                               }
                }
                if(_symtab[__var].bucket->isarray)
                   fprintf(_outfile,"\n\t.space\t%d",_symtab[__var].bucket->size-_symtab[__var].bucket->origsize);
            }
...
static inline void dumpquad()
{
    ...
    while(_quad)
    {
        ...
        switch(_quad->op)
        {
            ...
            case SUBOP: addsubop("sub"); break;
            case ADDOP: addsubop("add"); break;
...
unsigned short wincodegen()
{
    ...
    fprintf(_outfile,"#-----------------Intel x86 assembly code (AT&T syntax) - to be used with GNU assembler & linker (Windows NT - 32 bit only)-----------------#");
    fprintf(_outfile,"\n\n\t.file\t\"%s\"",_file);
    _logoplabel[2]='1',_freereg='b';
    dumpsymtab();   //dumping symbol table
    fprintf(_outfile,"\n\n\t.text\n\t.globl\t_main\n_main:");
    fprintf(_outfile,"\n\n# Printing epilog:\n\n\tpushl\t%sbp\n\tmovl\t%ssp, %sbp\n\n# Epilog finished",E,E,E); //printing epilog
